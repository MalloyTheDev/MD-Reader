...
Grab the latest signed Windows installer from the [**Releases**](https://github.com/MalloyTheDev/MD-Reader/releases) page:

- **[md-reader-1.4.2-setup.exe](https://github.com/MalloyTheDev/MD-Reader/releases/latest)** — current release

Then launch **MD Reader** and point it at any folder of `.md` files, or use the built-in vault.
...
### Rich Markdown
- GitHub-flavored Markdown with **KaTeX math** (per-equation *copy LaTeX* / *expand*)
...
- Safe file management — *Remove from Library* (undoable) vs *Delete* (to Recycle Bin), with confirmation
...
npm run build:win    # build the signed Windows installer → dist/
```
